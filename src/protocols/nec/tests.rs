use crate::{protocols::nec::{Nec, NecRaw}, bufrecv::BufferReceiver};
use crate::sender::PulseBuffer;
use crate::protocols::nec::cmds::{NecCommand, NecSamsungCommand, Nec16Command, NecAppleCommand};
...
#[test]
fn cmd_standard() {
    let cmd = NecCommand { addr: 7, cmd: 44, repeat: false };
    let bits: u32 = cmd.pack().into();
    ...
    let cmd2 = NecCommand::unpack(NecRaw {bits}, false).unwrap();
...
fn cmd_samsumg() {
    ...
    let cmd2 = NecSamsungCommand::unpack(bits.into(), false).unwrap();
    assert_eq!(cmd, cmd2);
}
...
fn cmd_nec16() {
    ...
    let cmd2 = Nec16Command::unpack(NecRaw {bits}, false).unwrap();
    assert_eq!(cmd, cmd2);
}
...
fn cmd_apple2009() {
    ...
    for (bits, cmdnum) in tests {
        ...
        let cmd = NecAppleCommand::unpack(NecRaw { bits: *bits }, false).unwrap();
